...
install-common:
	@ginstall -v -d "$(DESTDIR)$(MANDIR)/man1" && ginstall -m 0644 -v man/pass.1 "$(DESTDIR)$(MANDIR)/man1/pass.1"

	@[ "$(FORCE_BASHCOMP)" = "1" ] && ginstall -v -d "$(BASHCOMP_PATH)" && ginstall -m 0644 -v src/completion/pass.bash-completion "$(BASHCOMP_PATH)/pass" || true
	@[ "$(FORCE_ZSHCOMP)" = "1" ] && ginstall -v -d "$(ZSHCOMP_PATH)" && ginstall -m 0644 -v src/completion/pass.zsh-completion "$(ZSHCOMP_PATH)/_pass" || true
	@[ "$(FORCE_FISHCOMP)" = "1" ] && ginstall -v -d "$(FISHCOMP_PATH)" && ginstall -m 0644 -v src/completion/pass.fish-completion "$(FISHCOMP_PATH)/pass.fish" || true


ifneq ($(strip $(wildcard $(PLATFORMFILE))),)
install: install-common
	@ginstall -v -d "$(DESTDIR)$(LIBDIR)/password-store" && ginstall -m 0644 -v "$(PLATFORMFILE)" "$(DESTDIR)$(LIBDIR)/password-store/platform.sh"
	@ginstall -v -d "$(DESTDIR)$(BINDIR)/"
